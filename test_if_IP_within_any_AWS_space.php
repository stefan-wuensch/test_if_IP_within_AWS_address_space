...
// test_if_IP_within_any_AWS_space.php IPv4address [ IPv4address IPv4address IPv4address ... ]
//
// Usage with STDIN:
// some-command-generating-IPv4addresses | test_if_IP_within_any_AWS_space.php
// Multiple addresses per line are acceptable if delimited by commas or common whitespace
// characters like " ", "\n", "\t"
// 
// 
// Output:
// - If IP is found in AWS address space: 
// 	OK - AWS IP: [ the IP address ]
// - If IP is not found in AWS address space:
// 	Bad - not AWS IP: [ the IP address ]
// 
// Example output:
// Bad - not AWS IP:  5.175.193.164
// Bad - not AWS IP:  37.187.99.73
// OK - AWS IP:  54.182.204.72
// OK - AWS IP:  54.182.204.74
// 
//
...
// which don't fall within AWS address space.
// % awk '{print $3}' ELBlog.txt | cut -d: -f1 | sort -n | uniq | ./test_if_IP_within_any_AWS_space.php | grep -v OK
// Bad - not AWS IP:  5.175.193.164
// Bad - not AWS IP:  37.187.99.73
// Bad - not AWS IP:  54.80.106.124
// Bad - not AWS IP:  61.135.189.125
// Bad - not AWS IP:  61.160.247.231
// Bad - not AWS IP:  62.210.88.201
// Bad - not AWS IP:  66.249.66.2
// 
// =============================================================================================
...
foreach ( $OK_CIDRs_JSON -> prefixes as $CIDRblock ) {
	if ( $CIDRblock -> service == "AMAZON" ) {			// We only want CIDR blocks that are AWS
